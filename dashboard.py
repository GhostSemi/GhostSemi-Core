...
import sys
import subprocess
import psutil  # --- NEW: SYSTEM TELEMETRY ENGINE ---
import hashlib # --- NEW: SENTINEL ENCRYPTION ENGINE ---

# --- CORE v2.2 SECURITY: HARDWARE ID FINGERPRINT ---
def get_hwid():
...
class GhostDashboard(ctk.CTk):
    def __init__(self):
        ...
        # --- WINDOW CONFIG ---
        self.title("GhostSemi | Management Console v2.2")
        self.geometry("500x750") # Height increased to fit telemetry
        ...
        self.version_label = ctk.CTkLabel(self, text="SILICON INFRASTRUCTURE v2.2", font=("Courier", 10), text_color="#555")
    ...
    def update_telemetry(self):
        ...
        try:
            ...
            # If Pro is active, keep the bar neon blue
            if not self.is_pro:
    ...
    def check_persistence(self):
        ...
        if os.path.exists("pro_mode.txt"):
            with open("pro_mode.txt", "r") as f:
                ...
                # Verify if the stored hash matches this specific hardware
    ...
    def activation_success(self):
        ...
        # Sentinel: Save the encrypted hardware-locked hash
        secure_hash = generate_secure_token(self.current_hwid)
        with open("pro_mode.txt", "w") as f: 
            f.write(secure_hash)
            
        self.unlock_ui()
        messagebox.showinfo("GhostSemi v2.2", "Handshake Verified. System Optimized.")
    ...
    def hide_to_tray(self):
        ...
        self.icon_manager = pystray.Icon("GhostSemi", image, "GhostSemi v2.2", menu)
